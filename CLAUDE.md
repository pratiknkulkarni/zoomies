...
- `docs/spec/TECH_STACK.md` — authoritative for all technology choices
- `docs/spec/FEATURES.md` — authoritative for features and data model
- `docs/spec/DESIGN.md` — authoritative for visual design. **Every colour,
  spacing, radius, type size and duration comes from here. Never invent one.**
- `docs/history/PLAN.md` — current phase and task breakdown
...
Defined in `db/schema.ts`. Full detail in `docs/spec/FEATURES.md` §2.
...
ones surface as suggestions grouped by `family`. See `docs/spec/FEATURES.md` §3.
...
plugins/                Expo config plugins. `with-release-signing.js` puts the
                        release keystore into the generated build.gradle
media/                  README screenshots, generated by `scripts/screenshots.sh`
docs/                   A separate private repository — see § Repository
  spec/                 FEATURES, TECH_STACK, DESIGN — the authoritative three
  workflow/             RELEASING, SMOKE_TEST — how work happens
  history/              PLAN, DEVELOPMENT, ENGINEERING_NOTES — records, not rules
  SYNC.md               The one open proposal. Not scheduled, not built
```
...
- Every value comes from a token in `docs/spec/DESIGN.md`. Both of these are
  wrong: `style={{ color: '#1A1A17' }}` and `className="text-[#1A1A17]"`.
...
outer `.gitignore`. The two repositories commit separately:
...
- `docs/README.md` is the index. `spec/` decides things, `workflow/` says how
  work happens, `history/` is a record and settles nothing.
...
- **Follow the plan in `docs/history/PLAN.md`.** Do not jump ahead to a later
  phase.
...
- Do not scaffold features from `docs/spec/FEATURES.md` §14 (Deferred) or §15
  (Cut), even as placeholders.
...
See `docs/spec/FEATURES.md` §15 for the full list and reasoning.
