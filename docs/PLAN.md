...
> 234 unit tests.
>
> Merged to `main` 14 Aug 2026, with AC not formally reported — noted rather
> than assumed. AC3 (the first frame after a force-quit with an override set)
> and AC7 (`"value_num": null`, never `0`) are the two that would catch a real
> defect, and both still run against `main`.
>
> **Phase 11 — the last phase. Code complete 14 Aug 2026**, branch `phase-11`.
>
> **Two open questions closed by dissolving, not deciding.** §4.4 wanted a
> charting stack and got none: both charts are `View`s, because `DESIGN.md` §7
> forbids axes, gridlines, tooltips, gestures and animation, which is every
> feature such a library sells. And the factory reset is recorded as cut —
> Android's *Clear storage* already does it, *Clear cache* does nothing at all
> because the database is in the files directory, and iOS having no equivalent
> is written down as the condition that would reopen it.
>
> **`GRID_WEEKS` was a cap and is now a floor**, which is the whole of the
> scrolling change. `leading` falls to zero on its own once history passes
> thirteen weeks, so a young chart and a long one are one piece of arithmetic
> rather than two cases, and both `Math.max` clamps in `daysTrainedGrid` went
> with it. The renderer sizes the *cell* so a quarter spans the screen and the
> *content* to the data — two rules that were one, and both of the grid's
> layout bugs came from that.
>
> **The best-set trend is the last thing §10 was missing.** Dots never a line,
> and positioned by date rather than one per session — the second follows from
> the first, and dropping it would make a three-week gap look like three
> consecutive days. Its y labels are the all-time low and high, pinned outside
> the scroll, because an axis that moved with the window would make one dot
> height mean two numbers.
>
> The week geometry both charts stand on — `weekWindow`, `monthSpans`,
> `WINDOW_WEEKS` — moved to `lib/days.ts` rather than being copied, so a
> screenful is a quarter in both.
>
> **The mark is a `Z` in Geist 600**, generated by `scripts/icons.py` from the
> tokens read out of `global.css`. The wordmark `DESIGN.md` §12 proposed lost to
> Android's circular splash mask, verified by compositing the mask rather than
> by reading the spec. `app.json`'s splash grounds are corrected — they had
> carried the pre-Phase-8b palette since the refit.
>
> 254 unit tests.
>
> **Next, and none of it is code:** `SMOKE_TEST.md` AD on the Pixel 7a, which
> needs a database with more than thirteen weeks of history or every check in it
> passes for the wrong reason. Then `docs/STORE.md` §1 — the three long poles,
> all overdue — and the twelve DoD items walked end to end from a fresh install
> in airplane mode.
...
**Built, beyond the list above.** Two things the phase found rather than
planned:

- **Both charts scroll.** Phase 9 recorded that the grid does not scroll back
  past thirteen weeks because "a year would need horizontal paging, which is a
  gesture on a screen that currently has none." That was too strong — *paging*
  is a custom gesture; a horizontal `ScrollView` nested in a vertical one is
  not. The cap became a floor and the geometry moved to `lib/days.ts`, shared
  by the trend.
- **The factory reset**, asked for and answered in `FEATURES.md` §15 without
  building it. Android already has one; iOS does not, and that is the recorded
  trigger for reopening.

**Prepared, not finished.** `docs/PRIVACY.md` and `docs/STORE.md` hold the
policy text, both disclosure forms, listing copy and the screenshot list.
Everything left needs a person: hosting, a developer account, twelve testers,
and a device.
...
| Aug 2026 | Phase 11 built, and the plan closes. §4.4 settled by dissolving rather than deciding — both halves named a package and neither package is in the tree. Two things the phase found rather than planned: **both charts scroll**, after Phase 9's "a year would need horizontal paging, which is a gesture on a screen that currently has none" turned out to be too strong — *paging* is a custom gesture, a horizontal `ScrollView` nested in a vertical one is not — and the **factory reset**, asked for and answered in `FEATURES.md` §15 without building it. The week geometry both charts stand on moved to `lib/days.ts` rather than being copied. `docs/PRIVACY.md` and `docs/STORE.md` added: the policy text, both disclosure forms, listing copy and the screenshot list, so that a submission form is filled in by copying rather than decided at the keyboard. Everything remaining needs a person rather than a commit — hosting, a developer account, twelve testers and a device. |
